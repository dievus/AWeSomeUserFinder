...
def bucket_check(s3_client, args):
    try:
        ...
        if e.response["Error"]["Code"] == "AccessDenied":
            print(
                f"IAM user account is missing the s3:GetBucketPolicy permission. Review the example policy in the repo and modify appropriately. Quitting..."
            )
            quit()
...
def revert_bucket_policy(s3_client, bucket_resource, args):
    ...
    try:
        s3_client.put_bucket_policy(Policy=policy_document_str, Bucket=args.bucket)
