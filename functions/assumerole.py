...
def permissions_check(iam_client, args):
    try:
        ...
        print('The role provided is not valid. Either provide a valid role, or generate one with updateassumerolepolicygenerator.py.')
        quit()
    except ParamValidationError:
        print('You did not provide a role name as an option. Try again with a valid role.')
        quit()
    except ClientError as e:
        if e.response["Error"]["Code"] == "AccessDenied":
            print(f'IAM user account is missing the iam:GetRole permission. Review the example policy in the repo and modify appropriately. Quitting...')
            quit()
